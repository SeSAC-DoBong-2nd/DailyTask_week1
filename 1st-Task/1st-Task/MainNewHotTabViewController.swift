...
class MainNewHotTabViewController: UIViewController {
    ...
    @IBAction func topBtnAction(_ sender: UIButton) {
        ...
        let subtitleCommentsArr = ["다른 영화, 시리즈, 배우, 감독 또는 장르를 검색해 보세요.", "한식, 양식, 중식, 일식 등 메뉴를 검색해 보세요.", "버스, 택시, 지하철 등 이동할 대중교통을 검색해 보세요."]
        
        titleCommentLabel.text = titleCommentsArr.randomElement()
        subtitleCommentLabel.text = subtitleCommentsArr.randomElement()
    }
}
